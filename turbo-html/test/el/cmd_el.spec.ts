...
describe('[Commands for el]', function () {
  ...
  it('Update', async function () {
    nock('http://localhost')
      ...
      .reply(200, function (uri, requestBody) {
        return [commands.Update({ id: this.req.headers['x-ignis-output-id'], html: view_result(this.req.headers['x-ignis-output-id'], requestBody as any) })];
      });
  ...
  it('Remove', async function () {
    nock('http://localhost')
      .post('/api/book/form')
      .reply(200, function (uri, requestBody) {
        return [commands.Remove(this.req.headers['x-ignis-output-id'])];
      });
  ...
  it('AppendToEnd', async function () {
    ...
    nock('http://localhost')
      .post('/api/book/form')
      .reply(200, function (uri, requestBody) {
        return [commands.AppendToEnd({ id: this.req.headers['x-ignis-output-id'], html: view_result(id_new_el, data), })];
      });
