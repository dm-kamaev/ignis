...
export default class Manager {
  ...
  private _parse_cmd(cmd: string) {
    ...
    if (alias.Keydown.is(name)) {
      ...
      if (!method_url) {
        throw new Error(`[@ignis-web/html]: Incorrect address: "${method_url}" in cmd "${cmd}"`);
      }
      ...
      if (!m[1]) {
        throw new Error(`[@ignis-web/html]: Incorrect method in address: "${method_url}" in cmd "${cmd}"`);
      }
      if (!m[2]) {
        throw new Error(`[@ignis-web/html]: Incorrect url in address: "${method_url}" in cmd "${cmd}"`);
      }
      ...
      list_mod.forEach(el => {
        if (Once.is(el)) {
          ...
          throw new Error('[@ignis-web/html]: Unknown modificator '+el+', command = '+cmd);
        }
      });
