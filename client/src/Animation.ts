...
export default class Animation {
  on_update($el: HTMLElement) {
    ...
    if (!class_name) {
      throw new Error(`[@ignis-web/html]: Invalid ${enum_attr.ANIMATION.ON_UPDATE}="${$el.getAttribute(enum_attr.ANIMATION.ON_UPDATE)}", you must set class`);
    }
  ...
  on_remove($el: HTMLElement, cb: (...arg: any) => void) {
    ...
    if (!class_name) {
      throw new Error(`[@ignis-web/html]: Invalid ${enum_attr.ANIMATION.ON_REMOVE}="${$el.getAttribute(enum_attr.ANIMATION.ON_REMOVE)}", you must set class`);
    }
    if (!str_delay) {
      throw new Error(`[@ignis-web/html]: Invalid ${enum_attr.ANIMATION.ON_REMOVE}="${$el.getAttribute(enum_attr.ANIMATION.ON_REMOVE)}", you must set delay`);
    }
