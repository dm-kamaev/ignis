...
describe('[Alias keyboard]', function () {
  ...
  it('@keydown(enter, click) - revoke alias', async function () {
    ...
    nock('http://localhost')
      ...
      .reply(200, function (uri, requestBody) {
        count++;
        return view_btn(`click->POST:/api/book/revoke`, { id: this.req.headers['x-i-output-id'], output_id: this.req.headers['x-i-output-id'], data: null });
      });
  ...
  it('@keydown(enter, click) - revoke source event', async function () {
    ...
    nock('http://localhost')
      ...
      .reply(200, function (uri, requestBody) {
        count++;
        return view_btn(`@keydown(enter, click)`, { id: this.req.headers['x-i-output-id'], output_id: this.req.headers['x-i-output-id'], data: null });
      });
